...
(function(){{
  ...
  /* ── Performance Filters ── */
  var _nearStd='none',_nearStdWindow=2.0,_pbFilterActive=false;
  function applyPerformanceFilters(){{
    var nearActive=_nearStd!=='none';
    if(!nearActive&&!_pbFilterActive) return;
    document.querySelectorAll('tbody tr:not(.row-hidden)').forEach(function(row){{
      var show=true;
      if(nearActive&&show){{
        var target='→ '+_nearStd;
      ...
      if(_pbFilterActive&&show){{
        if(!row.querySelectorAll('td:not(.col-hidden) .progress-arrow:not(.hidden)').length) show=false;
      }}
  ...
  window.toggleArrows=function(btn){{
    ...
    document.querySelectorAll('.progress-arrow,.gap-next,.qual-ind').forEach(function(el){{el.classList.toggle('hidden',hidden);}});
    btn.textContent=hidden?'Show markers':'Hide markers';
    if(_nearStd!=='none'||_pbFilterActive) applyRowFilters();
  }};
  var _initSticky;
...
<div class="filter-bar">
  ...
  <div id="filter-rows">
    ...
    <div class="filter-row" id="perf-filter-row">
      ...
      <label class="perf-toggle" style="margin-left:16px"><input type="checkbox" id="pb-filter-cb" onchange="onPbFilterToggle(this)"> ▲ PB at most recent meet</label>
    </div>
  </div>
</div>
